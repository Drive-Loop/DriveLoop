...
def test_transcribe_endpoint_normalizes_driving_asr_terms(monkeypatch, tmp_path):
    class DummyProvider:
        def transcribe_file(self, audio_path, filename=None, content_type=None):
            from driveloop.intent.providers import (
                TranscriptionResult,
                normalize_driving_asr_transcript,
            )

            raw = (
                "Generate a 4G night intersection where a cyclist cuts in from "
                "the left and a pedestrian crosses in front of the Ago Bay High Court."
            )
            transcript, rules = normalize_driving_asr_transcript(raw)
            return TranscriptionResult(
                ...
                metadata={
                    ...
                    "normalized": transcript != raw,
                },
            )

    from driveloop.api import server

    server.set_asr_provider_for_testing(DummyProvider())
    try:
        response = client.post(
            "/transcribe",
            files={"audio": ("test.wav", b"not real wav but provider is mocked", "audio/wav")},
        )
    finally:
        server.set_asr_provider_for_testing(None)

    assert response.status_code == 200
    data = response.json()
    assert "foggy night intersection" in data["transcript"]
    assert "ego vehicle" in data["transcript"]
    assert data["metadata"]["raw_transcript"].startswith("Generate a 4G night")
    assert data["metadata"]["normalized"] is True
    assert data["metadata"]["normalization_rules"]
