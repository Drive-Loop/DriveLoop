...
def test_far_entry_numeric_start_offset(monkeypatch):
    from driveloop.actor_motion import build_actor_motion_plan

    monkeypatch.setenv("DRIVELOOP_EGO_FAR_ENTRY", "15")
    frames = build_actor_motion_plan(
        actor_controls=[{"actor_id": "actor_01", "category": "motorcycle"}],
        relations=["left"],
        motion_primitives=["cut_in"],
        executable_controls={},
    )["runtime_surface"]["frames"]
    assert frames[0]["longitudinal_offset_m"] == 15.0
    assert frames[-1]["longitudinal_offset_m"] == -4.0
