...
def test_smoke_suite_summary_exposes_condition_trace(tmp_path):
    ...
    assert summary["executable_condition"]["target_backend"] == "drivedreamer2_runtime"
    assert summary["executable_condition"]["trace_metadata"]["tensor_control_ready"] is True
    assert summary["executable_condition"]["trace_metadata"]["actor_motion_surface_ready"] is True
    assert (
