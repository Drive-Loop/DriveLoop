# DriveLoop
Closed-loop driving-scene generation and evaluation built on top of ChatSim.

DriveLoop is an extended research codebase based on the original ChatSim project. In addition to the original prompt-driven scene editing and rendering pipeline, this repository adds:

- a Streamlit frontend for interactive use
- a FastAPI service layer with a task queue
- browser microphone and audio-file prompt input
- multimodal prompt conditioning
- long-tail scenario control
- closed-loop evaluation and prompt refinement
- MagicDrive-V2 baseline integration

Original ChatSim resources:
...
```bash
git clone https://github.com/tangzimo0308/DriveLoop.git --recursive
```

Most of the low-level rendering, data preparation, and background-model setup steps below are inherited from ChatSim. The added DriveLoop features are documented in the Usage section and the docs under `docs/`.
...
Compared with the original ChatSim release, the recommended entry points in this repository are:

- `frontend/app.py` for the web UI
- `service/api.py` for the backend service API
- `main.py` for CLI generation, evaluation, and baseline experiments
- `docs/frontend_usage.md` for the frontend workflow
- `docs/service_usage.md` for the service API
- `docs/closed_loop_usage.md` for closed-loop evaluation and the MagicDrive-V2 baseline

### Web Frontend

The project now includes a Streamlit frontend with text input, browser microphone capture, audio upload, reference-file upload, and video preview.
Start the FastAPI service first, then launch Streamlit:
...
See [docs/frontend_usage.md](docs/frontend_usage.md) for details.

### Service API

The project also includes a FastAPI service layer with a built-in task queue:

```bash
uvicorn service.api:app --host 0.0.0.0 --port 8000
```

See [docs/service_usage.md](docs/service_usage.md) for the endpoints and upload fields.
...
This repository is built on top of the original ChatSim project. The underlying scene simulation, rendering, and data-processing pipeline come from ChatSim, while DriveLoop adds the closed-loop evaluation framework, service/frontend stack, multimodal prompt-conditioning workflow, and baseline experiment harness.

If you use this repository in academic work, please cite the original ChatSim paper and clearly state which parts come from ChatSim and which parts are your own extensions.
