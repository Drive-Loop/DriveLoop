...
def sanitize_filename(text, default="simulation", max_length=80):
    """Create a portable filename stem from a natural-language prompt."""
...
def generate_video(scene, prompt, save_images=False):
    ...
    filename = sanitize_filename(prompt, max_length=80)
    video_path = os.path.join(video_output_path, f"{filename}.mp4")
    fps = scene.fps
...
def scale_dense_depth_map(dense_depth_map, sparse_depth_map, depth_map_mask):
    ...
    # Extract valid pixels from the dense and sparse depth maps.
...
def srgb_gamma_correction(linear_image):
    ...
    linear_image: np.ndarray
        shape: H*W*C
    """
    linear_image = np.clip(linear_image, 0, 1)  # Clamp values to [0, 1].
    gamma_corrected_image = np.where(linear_image <= 0.0031308,
                          linear_image * 12.92,
                          1.055 * (linear_image ** (1 / 2.4)) - 0.055)
    gamma_corrected_image = np.clip(gamma_corrected_image, 0, 1)  # Clamp values to [0, 1].
